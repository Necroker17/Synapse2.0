...
>
> **Fuente de verdad de los datos duros:** `Landing_SynapseAI/lib/stats.ts` (snapshot de telemetría vigente). Producto real: **Oro (XAUUSD), exclusivamente M15**, con **dos sesiones diarias independientes**: Nueva York 7:00–11:00 a.m. y Asia 6:00–10:00 p.m. (hora Colombia, UTC-5). La Plata fue deshabilitada — no se menciona como activo operable.

---

## 1. Qué es

**Synapse AI Indicator** es un indicador/escáner algorítmico para TradingView, de acceso Invite-Only, que analiza en tiempo real el par **XAUUSD en temporalidad M15** durante **dos ventanas horarias diarias — la apertura de Nueva York y la sesión de Asia** — y le entrega al trader una señal completa y accionable: dirección, punto de entrada, Stop Loss, dos Take Profit (TP1/TP2) y el tamaño de posición exacto según su capital y riesgo definidos.
...
- **Detecta la señal:** escanea cada vela de M15 en XAUUSD durante las sesiones de Nueva York y de Asia, y determina si existe una oportunidad de entrada (compra o venta) en cada una.
- **Etiqueta la señal por sesión:** cada operación queda marcada como `NY` o `ASIA` según la ventana en la que se generó, y el trader puede seguir la de cualquiera de las dos.
...
- **Muestra su propio historial de desempeño:** un panel en el gráfico con señales registradas, win rate a TP1, win rate a TP2, profit factor, racha máxima de aciertos/fallos y retorno acumulado — desglosado también por sesión (NY vs. Asia) — para que el trader pueda auditar el sistema, no solo confiar en él a ciegas.
...
1. **Filtro de ventana horaria.** Concentra el análisis en **dos ventanas de mayor volumen institucional** (hora Colombia): **Nueva York, 7:00–11:00 a.m.**, y **Asia, 6:00–10:00 p.m.** Cada ventana corre el mismo análisis de forma independiente y puede generar su propia señal, etiquetada `NY` o `ASIA`. Fuera de esas dos ventanas no evalúa señales nuevas, para no operar en tramos de baja liquidez donde el ruido genera falsos positivos. En la práctica, esto le da al trader dos oportunidades por día en vez de una — si no alcanzó a tomar la señal de una sesión, todavía tiene la otra el mismo día.
...
- **Dependencia de estar frente a la pantalla:** gracias a las alertas al celular, el trader puede enterarse de una señal válida sin tener que vigilar el gráfico en vivo durante ninguna de las dos sesiones.
- **Horarios que no le sirven a todos:** un trader que trabaja en horario de oficina y no puede seguir la sesión de Nueva York en vivo todavía tiene la sesión de Asia en la noche — y viceversa.

## 6. Cómo impacta la vida de un trader

- **Recupera tiempo:** no necesita quedarse pegado a la pantalla durante las cuatro horas de NY ni las cuatro de Asia; la notificación llega a su celular cuando hay algo que revisar.
- **Se adapta a su horario, no al revés:** si su rutina no le permite estar disponible en la mañana (NY), puede seguir operando de noche (Asia) sin perderse el día — y si se perdió una señal en una sesión, la otra sigue en pie.
